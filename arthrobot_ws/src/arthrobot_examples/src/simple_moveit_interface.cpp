...
void move_robot(const std::shared_ptr<rclcpp::Node> node)
{
    ...
    std::vector<double> arm_joint_goal {1.57, 0.0, 0.0, -0.5};  // joint values
    std::vector<double> gripper_joint_goal {0.2};
    ...
    if (!arm_within_bounds | !gripper_within_bounds)
    {
        RCLCPP_WARN(rclcpp::get_logger("rclcpp"),
                    "Target joint position(s) were outside of limits, but will plan and clamp to the limits ");
        return;
    }
    ...
    if(arm_plan_success && gripper_plan_success)
    {
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                    "Planner SUCCEEDED, moving the arm and the end effector");
        arm_move_group.move();
